...
def split_spatial_dataset_patches(imageSource, annotationFile, destinationFolder, patchSize, jitter=(0, 0),  force=False):
    '''
    ...
    for fIdx in tqdm(range(in_layer.GetFeatureCount())):
        ...
        def export_polygon(poly, image_index, annotation_index):
            ...
            meta_out['annotations'].append({
                'id': annotation_index,
                'bbox': poly_extent,
                'area': 0.5*np.abs(np.dot(poly_np[:,0],np.roll(poly_np[:,1],1))-np.dot(poly_np[:,1],np.roll(poly_np[:,0],1))),      # shoelace formula
...
if __name__ == '__main__':
    ...
    split_spatial_dataset_patches(args.image_source, args.annotation_file, args.dest_folder,
                                    args.patch_size, args.jitter, bool(args.force))